...
const supabase = createClient(
  "https://xazjdguewshbmfiwshce.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhhempkZ3Vld3NoYm1maXdzaGNlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ5ODYyNTQsImV4cCI6MjA5MDU2MjI1NH0.exEZhIPQDcSGve7PG0-ml9zMVxNX83nWmZfVv-6kDgE"
);

// Court number → Supabase UUID mapping
const COURT_UUID = {
  1: "4c1e0905-f260-4a5e-a12f-6dcd9080dec8",
  2: "9ccbeefe-f63e-4a53-9064-535293f9cc29",
  3: "3a44bad6-0c19-490c-b1bb-32d4e0675e65",
  4: "8df1f1ba-a6bf-4f55-a0fa-c782fd2a73f8",
  5: "14c5f046-1eb7-472c-b06c-c3becde42fc0",
  6: "ded7546c-7ca1-4e90-9599-46367efc638c",
  7: "804259de-5813-47df-99f8-1d303674f0c7",
  8: "359fa3de-62df-44fd-be20-526d35a9a892",
};

// UUID → court number (reverse lookup)
const UUID_TO_COURT = Object.fromEntries(Object.entries(COURT_UUID).map(([k,v]) => [v, Number(k)]));
...
const PARK = {
  name: "Marine Park",
  address: "Ave U & Stuart St, Brooklyn, NY",
...
const INIT_COURTS = [
  ...
  { id: 2, type: "beginner", stacks: 0, conditions: [], lastReport: null, reporter: null },
  { id: 3, type: "regular", stacks: 1, conditions: [], lastReport: "3m ago", reporter: "Sarah K." },
  { id: 4, type: "regular", stacks: 2, conditions: [], lastReport: "1m ago", reporter: "Dave R." },
  { id: 5, type: "regular", stacks: 0, conditions: [], lastReport: null, reporter: null },
  { id: 6, type: "regular", stacks: 1.5, conditions: ["windy"], lastReport: "8m ago", reporter: "Lisa M." },
  { id: 7, type: "challenge", stacks: 3, conditions: [], lastReport: "2m ago", reporter: "Tony B." },
...
const INIT_REPORTS = [
  ...
  { id: 2, court: 7, stacks: 3, conditions: [], user: "Tony B.", time: "10:39 AM", text: "Challenge courts are packed today" },
  ...
  { id: 5, court: 8, stacks: 0.5, conditions: [], user: "Mike T.", time: "10:29 AM", text: "" },
];
...
const getCourtFill = (stacks) => {
  if (stacks === 0) return "#4CAF50";
  if (stacks <= 1) return "#FFC107";
  if (stacks <= 2) return "#FF9800";
  return "#F44336";
};

const formatStacks = (n) => {
  if (n === 0) return "0";
  if (n % 1 === 0.5) return n.toString();
  return n.toString();
};
...
export default function CourtPulse() {
  ...
  // ─── Helper: DB row → chat message ───
  const dbToChat = useCallback((row, currentName) => ({
    id: row.id,
    name: row.user_name === currentName ? "You" : row.user_name,
    text: row.text,
    time: new Date(row.created_at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }),
    self: row.user_name === currentName,
  ...
  // ─── Load chat messages from Supabase & subscribe to realtime ───
  useEffect(() => {
    if (!userName) return;

    // Load main park chat (court_id IS NULL)
    supabase
      .from("chat_messages")
      .select("*")
      ...
      .then(({ data }) => {
        if (data) setMainChat(data.map(r => dbToChat(r, userName)));
      });

    // Load court-specific chats
    supabase
      .from("chat_messages")
      .select("*")
      .eq("park_id", "marine-park")
      ...
      .then(({ data }) => {
        if (data) {
          ...
          data.forEach(r => {
            const courtNum = UUID_TO_COURT[r.court_id];
            if (courtNum) grouped[courtNum].push(dbToChat(r, userName));
          });
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chat_messages" }, (payload) => {
        ...
        if (row.park_id !== "marine-park") return;
        const msg = dbToChat(row, userName);
        if (!row.court_id) {
          setMainChat(p => {
          ...
          const courtNum = UUID_TO_COURT[row.court_id];
          if (courtNum) {
            setCourtChats(p => {
              const existing = p[courtNum] || [];
              if (existing.some(m => m.id === row.id)) return p;
              return { ...p, [courtNum]: [...existing, msg] };
            });
          }
        }
      })
  ...
  const sendCourtChat = useCallback(async () => {
    ...
    const { error } = await supabase.from("chat_messages").insert({
      park_id: "marine-park",
      court_id: COURT_UUID[selectedCourt],
      user_name: userName,
      text,
    });
    ...
    return (
      <div style={{ ...s.wrap, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, background: `linear-gradient(180deg, ${C.banner} 0%, #264D73 40%, ${C.bg} 100%)`, minHeight: "100vh" }}>
        ...
        <div style={{ width: "100%", maxWidth: 320 }}>
          ...
          />
          <input
            value={contactInput} onChange={e => setContactInput(e.target.value)}
  ...
  // ═══════════════════════════════════════════════════════════════════════
  const courtMap = (() => {
    const w = 380, h = 260, cW = 76, cH = 82, gap = 10;
    const startX = (w - (cW * 4 + gap * 3)) / 2;
    const topY = 22;
    const botY = topY + cH + 34;
    ...
    return (
      <div style={{ ...s.section, marginTop: 16 }}>
        <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{ display: "block" }}>
          ...
          {/* Net dividers */}
          <line x1={startX-6} y1={topY+cH+17} x2={startX+cW*4+gap*3+6} y2={topY+cH+17} stroke="#5A6270" strokeWidth="1.5" strokeDasharray="8,5" opacity="0.18"/>
          <text x={w/2} y={topY+cH+13} textAnchor="middle" fontSize="7.5" fill="#5A6270" fontFamily={ff} opacity="0.45" fontWeight="600">NET DIVIDERS</text>
          ...
          {positions.map(pos => {
            ...
            const fill = getCourtFill(court.stacks);
            const hasCond = court.conditions.length > 0;

            return (
              <g key={pos.id} onClick={() => setSelectedCourt(pos.id)} style={{ cursor: "pointer" }}>
                <rect x={x} y={y} width={cW} height={cH} rx="8" fill={fill} opacity="0.15" stroke={fill} strokeWidth="1.5"/>
                <rect x={x+3} y={y+3} width={cW-6} height={cH-6} rx="6" fill={fill} opacity="0.08"/>
                {/* Court lines */}
                <line x1={x+cW/2} y1={y+10} x2={x+cW/2} y2={y+cH-10} stroke={fill} strokeWidth="0.8" opacity="0.3"/>
                <line x1={x+12} y1={y+cH/2} x2={x+cW-12} y2={y+cH/2} stroke={fill} strokeWidth="0.8" opacity="0.3"/>
                {/* Court number */}
                <text x={x+cW/2} y={y+cH/2-6} textAnchor="middle" fontSize="20" fontWeight="800" fill={fill} fontFamily="'Fraunces', serif" opacity="0.9">{pos.id}</text>
                {/* Type label */}
                <text x={x+cW/2} y={y+cH/2+12} textAnchor="middle" fontSize="7" fontWeight="700" fill={fill} fontFamily={ff} opacity="0.6" letterSpacing="0.8">
                  {court.type === "challenge" ? "CHALLENGE" : court.type === "beginner" ? "BEGINNER" : "REGULAR"}
                </text>
                {/* Condition dot */}
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 10, flexWrap: "wrap" }}>
          {[
            { color: "#4CAF50", label: "Empty" },
            { color: "#FFC107", label: "≤1 Stack" },
            { color: "#FF9800", label: "2 Stacks" },
            { color: "#F44336", label: "3+ Stacks" },
          ].map(l => (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
            {OVERALL_LEVELS.map((lvl, i) => {
              ...
              <button key={lvl.key} onClick={() => setParkReportLevel(i)} style={{
                ...
                {selected && (
                  <div style={{ width: 22, height: 22, borderRadius: 11, background: "rgba(255,255,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800 }}>✓</div>
                )}
                {!selected && (
                  <div style={{ width: 10, height: 10, borderRadius: 5, background: lvl.color, opacity: 0.5 }}/>
                )}
              </button>
              );
            })}
    ...
    <div key={msg.id} style={{ display: "flex", flexDirection: msg.self ? "row-reverse" : "row", gap: 6, marginBottom: 6, alignItems: "flex-end" }}>
      <div style={{ maxWidth: "78%" }}>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ padding: "10px 14px", borderRadius: msg.self ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: msg.self ? C.chatSelf : C.chatOther, color: msg.self ? "#fff" : C.text, fontSize: 14, lineHeight: 1.4, border: msg.self ? "none" : `1px solid ${C.border}` }}>
            {msg.flagged ? <span style={{ fontStyle: "italic", opacity: 0.5 }}>This message has been reported</span> : msg.text}
          </div>
          {/* Flag button — only on others' messages, not already flagged */}
          {!msg.self && !msg.flagged && (
            <button onClick={() => handleFlag(msg.id, chatType)} style={{ position: "absolute", top: -6, right: -6, width: 22, height: 22, borderRadius: 11, background: C.bg, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", opacity: 0.5 }} title="Report message">
              {Icons.Flag(10)}
            </button>
          )}
        </div>
  ...
  const chatScreen = (
    <div style={{ paddingBottom: 90, display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      <div style={{ padding: "8px 16px", borderTop: `1px solid ${C.border}`, background: C.card, display: "flex", gap: 8, alignItems: "center" }}>
        <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMainChat()} placeholder="Message..." style={{ flex: 1, padding: "10px 14px", borderRadius: 20, border: `1px solid ${C.border}`, fontSize: 14, fontFamily: ff }} />
        ...
        <div style={{ width: "100%", maxWidth: 430, margin: "0 auto", background: C.card, borderRadius: "20px 20px 0 0", padding: "20px 20px 32px", animation: "slideUp 0.25s ease" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginBottom: 8 }}>
            ...
            <div style={{ minWidth: 70, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: sc.color, marginTop: -2 }}>{sc.label}</div>
            </div>
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: C.bg, zIndex: 150, display: "flex", flexDirection: "column", maxWidth: 430, margin: "0 auto" }}>
        ...
        <div style={{ padding: "8px 16px env(safe-area-inset-bottom, 8px)", borderTop: `1px solid ${C.border}`, background: C.card, display: "flex", gap: 8, alignItems: "center" }}>
          <input value={courtChatInput} onChange={e => setCourtChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendCourtChat()} placeholder={`Message Court ${selectedCourt}...`} style={{ flex: 1, padding: "10px 14px", borderRadius: 20, border: `1px solid ${C.border}`, fontSize: 14, fontFamily: ff }}/>
